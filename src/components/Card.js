...
function CardWishlist({item}) {
    ...
    return (
        <div className='card-div cardWishlist-div'>
            ...
            <button style={{backgroundColor: 'rgb(255, 203, 255)', color: "rgb(46, 30, 47)"}} className='card-button cardWishlist-button' onClick={() => dispatch(deleteFromWishlist(item))}>{'Delete'}</button>  
        </div>
    )
}
